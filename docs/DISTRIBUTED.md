...
> 状态：**设计稿，未实现**。依赖 DESIGN.md 的单机恢复契约。
> 本文定义对等抢占模式；SCHEDULER.md 定义中心指派模式。一个集群只启用一种模式。
> 事件模型、fold 和 DAG 语义可以复用；租约、持久信号与副作用边界需要新增协议。
...
验证新请求和恢复共同受容量约束、租约轮换不漏事件、订阅断线可补齐。
多节点压测记录吞吐与 DB 瓶颈，不以“近似均匀”代替每个 run 的正确性验收。
