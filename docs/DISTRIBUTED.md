...
不可重试失败都必须处理。delay 仍按单机约定重放全时长；重试退避同样等满整段
`backoff_ms`（事件 ts 来自写入方时钟，接管方时钟不同源，剩余时间减法不可靠）。
...
seq 决定事件顺序；ts 只用于展示，**不**参与剩余退避计算（等价于续算：
事件 ts 来自写入方时钟，跨节点恢复时与调度方时钟不同源，减法不可靠，
所以恢复一律等满整段 backoff），也不决定哪个节点已经完成。
